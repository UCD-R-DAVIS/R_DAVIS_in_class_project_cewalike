...
source("scripts/week_8_function.R")
...
gapminder %>% 
  filter(country == "Canada", year %in% c(1950:1978)) %>%
...
# Challenge ---------------------------------------------------------------
# Write a new function that takes two arguments, 
# the gapminder data.frame (d) and the name of a country (e.g. "Afghanistan"), 
# and plots a time series of the country’s population. 
# The return value from the function should be a ggplot object. 
# Note: It is often easier to modify existing code than to start from scratch. 
# To start out with one plot for a particular country, 
# figured out what you need to change for each iteration 
# (these will be your arguments), and then wrap it in a function.
library(tidyverse)
library(ggplot2)

Pop_plot <- function(d, cntry) {
  ggplot(d[d$country %in% c(cntry),], aes(x= year, y= pop)) +
    geom_line() +
    labs(x = "Year", y = "Population") +
    theme(axis.title = element_text(size = 12, color = "black"))
}

Pop_plot(gapminder, "United States")
